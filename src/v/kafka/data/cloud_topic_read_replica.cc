...
partition_proxy::get_snapshot() const {
    ...
    // Read the source's migration phase from the snapshot. While the source is
    // still migrating (tiered->cloud), its data lives in L1 as imported extents
    // (the dark mirror); the L1 read path below serves those exactly as it
    // serves native extents, so no separate tiered-storage read path is needed.
    // The phase is surfaced for observability and to gate behavior at cutover.
    auto phase = l1::migration_phase::none;
    auto offs = co_await snapshot_res->metastore->get_offsets(metadata->tidp);
    if (offs.has_value()) {
        phase = offs->migration_phase;
    }
    vlog(
      cd_log.debug,
      "read replica {} snapshot: source migration_phase={}",
      ntp(),
