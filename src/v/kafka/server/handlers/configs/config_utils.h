...
inline bool is_storage_mode_transition_permitted(
  ...
  bool migration_enabled) {
    ...
    // topic_storage_mode_migration cluster feature is active (so a
    // mixed-version cluster does not begin a migration older nodes cannot
    // drive) AND the enable_topic_storage_mode_migration config is on.
    if (
...
struct storage_mode_validator {
    ...
    // the caller only when the topic_storage_mode_migration feature is active
    // AND the enable_topic_storage_mode_migration config is on. Permits those
    // transitions.
