...
inline bool is_storage_mode_transition_permitted(
  ...
  bool migration_enabled) {
    ...
    // by `migration_enabled`, which the caller sets only when the
    // tiered_to_cloud_migration cluster feature is active (so a mixed-version
    // cluster does not begin a migration older nodes cannot drive) AND the
    // enable_ts_tsv2_migration config is on.
    if (
...
struct storage_mode_validator {
    ...
    // the caller only when the tiered_to_cloud_migration feature is active AND
    // the enable_ts_tsv2_migration config is on. Permits those transitions.
