...
public:
    ...
    // (partition_storage_mode is still tiered, so cloud_topic_enabled() is
    // false until cutover). While served as tiered storage the archiver's
    // mirror -- not the reconciler -- owns the L1 region, so the reconciler
    // must skip it to avoid two writers; it resumes the round after cutover
    // advances partition_storage_mode.
