...
ss::future<> reconciler<Clock>::reconcile() {
    ...
    for (auto& [_, src] : _sources) {
        ...
        // plain tiered, or still migrating tiered->cloud
        // (partition_storage_mode is still tiered until cutover) -- has the
        // archiver's mirror as its sole L1 writer, so the reconciler must skip
        // it to avoid two writers. Re-evaluated every round, so a partition is
        // picked up automatically the round after cutover advances
