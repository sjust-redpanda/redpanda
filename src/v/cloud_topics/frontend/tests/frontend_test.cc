...
// Helper: create a tiered (not cloud-topic) partition and return the leader.
// ctp_stm is installed as a passenger, but on_ctp_partition_leader does not
// fire for non-CT partitions, so init_ts_ct_migration() never runs and the
// ts_migration_boundary remains nullopt.  Tests that need to set a specific
// boundary via start_ts_import() should use this helper to avoid races with
// the automatic background migration triggered for tiered_cloud partitions.
static ss::lw_shared_ptr<cluster::partition>
make_tiered_partition(frontend_fixture& fx, const model::topic& topic) {
    model::ntp ntp(model::kafka_namespace, topic, 0);
    cluster::topic_properties props;
...
TEST_F(frontend_fixture, passthrough_reader_routes_pre_migration_offset) {
    auto partition = make_tiered_partition(*this, model::topic{"pre_bound"});
    cloud_topics::frontend fe(partition, _data_plane.get());
...
TEST_F(frontend_fixture, passthrough_reader_not_invoked_above_boundary) {
    auto partition = make_tiered_partition(*this, model::topic{"above_bound"});
    cloud_topics::frontend fe(partition, _data_plane.get());
