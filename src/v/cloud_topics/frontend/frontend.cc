    ...
    if (ts_bound.has_value() && base <= *ts_bound) {
        ...
        vlog(
          ...
          *ts_bound,
          source.size());
        for (const auto& range : source) {
            // tx_range stores model::offset values (raft space after OT).
            // from_log_offset() returns model::offset; manifest arithmetic
            // (model::offset - model::offset_delta) yields kafka::offset which
            // we cast back to model::offset via kafka::offset_cast.
            model::offset kafka_last;
            if (range.last <= offsets.end_rp) {
                // Within the OT state coverage: use exact translation.
                kafka_last = ot_state->from_log_offset(range.last);
            } else {
                ...
                if (kafka_last > kafka::offset_cast(*ts_bound)) {
                    kafka_last = kafka::offset_cast(*ts_bound);
                }
            }
            auto kafka_first = ot_state->from_log_offset(
              std::max(offsets.begin_rp, range.first));
            vlog(
              cd_log.info,
              "  tx pid={} range.first={} range.last={} -> "
              "kafka_first={} kafka_last={} (begin_rp={} end_rp={})",
              range.pid,
              range.first,
              range.last,
              kafka_first,
              ...
              offsets.begin_rp,
              offsets.end_rp);
            target.emplace_back(range.pid, kafka_first, kafka_last);
        }
        co_return target;
    }
