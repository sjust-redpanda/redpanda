...
ss::future<> housekeeper::do_loop() {
    ...
    try {
        ...
        // (partition_mode lags at tiered until cutover) -- has an idle ctp_stm;
        // running housekeeping would force epoch/placeholder maintenance that
        // seeds a meaningless reconciled offset and pins local-log GC, freezing
        // retention. Skip until it becomes a cloud topic (cutover), which is
        // also where the reconciler picks it up.
