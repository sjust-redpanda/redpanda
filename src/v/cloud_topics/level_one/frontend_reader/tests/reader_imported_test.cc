...
TEST_F(ReaderImportedTest, ReadImportedExtentReturnsKafkaOffsets) {
    ...
    // delta=5: log offset N → kafka offset N - 5
    const model::offset_delta delta{5};

    iobuf seg;
    for (int i = 0; i < 5; ++i) {
        // log offset 5+i, delta=5 → kafka offset i
        seg.append(batch_to_ts_bytes(make_data_batch(
          kafka::offset{5 + i}, kafka::offset{5 + i})));
    }

    register_imported_extent(
      tidp, std::move(seg), 0_o, 4_o, delta, "t/seg-a.log")
      .get();
    ...
    for (int i = 0; i < 5; ++i) {
        EXPECT_EQ(batches[i].base_offset(), kafka::offset_cast(kafka::offset{i}))
          << "batch " << i;
    }
}
...
TEST_F(ReaderImportedTest, ReadImportedExtentRespectsStartOffset) {
    ...
    register_imported_extent(
      tidp, std::move(seg), 0_o, 4_o, delta, "t/seg-b.log")
      .get();
...
TEST_F(ReaderImportedTest, ReadImportedExtentSkipsNonDataBatches) {
    ...
    const model::offset_delta delta{0};
    ...
    register_imported_extent(
      tidp, std::move(seg), 0_o, 1_o, delta, "t/seg-c.log")
      .get();
...
TEST_F(ReaderImportedTest, ReadImportedExtentThenNative) {
    ...
    register_imported_extent(
      tidp, std::move(seg), 0_o, 4_o, delta, "t/seg-d.log")
      .get();
