...
struct imported_ts_segment_info
  : public serde::envelope<
    ...
    auto serde_fields() {
        return std::tie(delta_offset, delta_offset_end, segment_term);
    }
...
inline imported_ts_segment_info to_segment_info(const imported_ts_info& i) {
    return imported_ts_segment_info{
      .delta_offset = i.delta_offset,
      .delta_offset_end = i.delta_offset_end,
...
inline imported_ts_info to_imported_ts_info(
  ...
  kafka::offset last_kafka_offset) {
    return imported_ts_info{
      ...
      .delta_offset_end = seg.delta_offset_end,
...
// the extent row's segment descriptor, and the extent's last offset (location +
// segment are set together for an imported extent, both nullopt for native L1).
inline std::optional<imported_ts_info> to_imported_ts_info(
  ...
  kafka::offset last_kafka_offset) {
    if (loc.has_value() && seg.has_value()) {
        return to_imported_ts_info(*loc, *seg, last_kafka_offset);
    }
    return std::nullopt;
}
...
struct extent
  : public serde::
    ...
    std::optional<imported_ts_segment_info> imported_ts_delta;
