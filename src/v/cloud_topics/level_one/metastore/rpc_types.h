...
struct get_offsets_reply
  : serde::
    ...
    auto serde_fields() {
        return std::tie(ec, start_offset, next_offset, migration_phase);
    }
...
struct set_migration_phase_reply
  : serde::envelope<
      set_migration_phase_reply,
      serde::version<0>,
      serde::compat_version<0>> {
    auto serde_fields() { return std::tie(ec); }
...
struct set_migration_phase_request
  : serde::envelope<
      set_migration_phase_request,
      serde::version<0>,
      serde::compat_version<0>> {
    using resp_t = set_migration_phase_reply;
