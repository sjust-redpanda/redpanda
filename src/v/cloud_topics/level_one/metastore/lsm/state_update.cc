  ...
  const {
    ...
    for (const auto& [tidp, extents] : new_extents_by_tp) {
        ...
        verified_meta_vals[tidp] = metadata_row_value{
          ...
          .migration_phase = opt ? opt->migration_phase : migration_phase::none,
        };
    }
...
append_imported_objects_db_update::build_rows(
  state_reader& state, chunked_vector<write_batch_row>& out) const {
    ...
    chunked_hash_map<object_id, std::optional<imported_segment_info>>
      imported_by_oid;
    chunked_hash_map<object_id, size_t> total_by_oid;
    for (const auto& o : new_objects) {
        imported_by_oid[o.oid] = o.imported;
    }
    for (const auto& [tidp, extents] : new_extents_by_tp) {
    ...
    for (const auto& [tidp, extents] : new_extents_by_tp) {
        ...
        for (const auto& e : extents) {
            size_sum += e.len;
            auto imp_it = imported_by_oid.find(e.oid);
            out.emplace_back(
              write_batch_row{
                ...
                .value = serde::to_iobuf(
                  extent_row_value{
                    ...
                    .len = e.len,
                    .oid = e.oid,
                    .imported = imp_it != imported_by_oid.end() ? imp_it->second
                                                                : std::nullopt,
                  }),
              });
        }

        const auto current_phase = opt ? opt->migration_phase
          ...
          write_batch_row{
            ...
            .value = serde::to_iobuf(
              metadata_row_value{
                ...
                .migration_phase = current_phase == migration_phase::none
                                     ? migration_phase::migrating
                                     : current_phase,
              }),
          });
...
ss::future<std::expected<void, db_update_error>>
set_migration_phase_db_update::build_rows(
  state_reader& reader,
  chunked_vector<write_batch_row>& out,
  bool* is_no_op) const {
    auto meta_res = co_await reader.get_metadata(tp);
    ...
    // complete creates its metadata row (the migrating marker may be the
    // partition's first write).
    metadata_row_value updated = meta_res->has_value() ? **meta_res
                                                       : metadata_row_value{};
    const auto current = updated.migration_phase;
