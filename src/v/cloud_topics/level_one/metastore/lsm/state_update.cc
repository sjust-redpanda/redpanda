  ...
  const {
    ...
    for (const auto& [tidp, extents] : new_extents_by_tp) {
        ...
        if (opt) {
            ...
            // a TS-migration signal is present. See state_update.cc for the
            // full description of the two valid TS-migration cases.
            ...
            if (has_terms && terms_it->second.begin()->start_offset < base) {
              ...
              && terms_it->second.begin()->term_id > model::term_id{0}) {
                ...
                // will be counted as removed data in the alignment check below.
