...
add_objects_db_update::build_rows(
  ...
  const {
    ...
    for (const auto& [tidp, extents] : new_extents_by_tp) {
        ...
        // A fresh partition marked migrating (via set_migrating) adopts its log
        // at the first incoming extent's base, allowing a non-zero start (the
        // TS->CT mount); any other fresh partition must start at 0.
              ...
              write_batch_row{
                ...
                .value = serde::to_iobuf(
                  extent_row_value{
                    ...
                    .len = entry.len,
                    .oid = entry.oid,
                    .imported_ts_delta = entry.imported_ts_delta,
                  }),
              });
        }
    }
