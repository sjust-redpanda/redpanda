...
TEST(DebugReaderTest, RoundTripExtentValue) {
    ...
    EXPECT_FALSE(decoded->get_extent().has_imported_ts_delta());
}
...
TEST(DebugReaderTest, RoundTripExtentValueImported) {
    ...
    isi.set_delta_offset(7);
    isi.set_delta_offset_end(9);
    isi.set_segment_term(3);
    ev.set_imported_ts_delta(std::move(isi));
    val.set_extent(std::move(ev));
    ...
    ASSERT_TRUE(decoded.has_value());
    ASSERT_TRUE(decoded->has_extent());
    ASSERT_TRUE(decoded->get_extent().has_imported_ts_delta());
    const auto& d = decoded->get_extent().get_imported_ts_delta();
    EXPECT_EQ(d.get_delta_offset(), 7);
    EXPECT_EQ(d.get_delta_offset_end(), 9);
