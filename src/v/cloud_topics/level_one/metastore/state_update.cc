...
// The offset a batch of extents must connect at for a partition. An existing
// non-empty partition continues at its tail. A fresh partition that has been
// marked migrating (via set_migrating) adopts its log at the first incoming
// extent's base, allowing a non-zero start -- this is the TS->CT mount that
// imports a partition whose history still lives in tiered storage. Any other
// fresh partition must start at offset 0.
        ...
        for (const auto& [p, extent_meta] : p_extents) {
            ...
            ret_extents.insert(
              extent{
                ...
                .len = extent_meta.len,
                .oid = oid,
                .imported_ts_delta = extent_meta.imported_ts_delta,
              });
        }
    }
