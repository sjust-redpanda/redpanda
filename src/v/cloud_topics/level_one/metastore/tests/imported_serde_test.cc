...
TEST(ImportedSerdeTest, ExtentRowValueV0CompatV1) {
    ...
    ASSERT_EQ(v1.imported_ts_delta, std::nullopt);
}
...
TEST(ImportedSerdeTest, ExtentRowValueV1RoundTrip) {
    ...
    v1.imported_ts_delta = imported_ts_segment_info{
    ...
    ASSERT_TRUE(decoded.imported_ts_delta.has_value());
    ASSERT_EQ(decoded.imported_ts_delta, v1.imported_ts_delta);
}
