...
TEST(ImportedSerdeTest, ExtentRowValueV0CompatV1) {
    ...
    ASSERT_EQ(v1.imported, std::nullopt);
}
...
TEST(ImportedSerdeTest, ExtentRowValueV1RoundTrip) {
    ...
    v1.imported = imported_segment_info{
      .ts_path = "00000000000000000000-1-v1.log",
      .delta_offset = model::offset_delta{5},
      .delta_offset_end = model::offset_delta{5},
      .base_kafka_offset = kafka::offset{95},
      .last_kafka_offset = kafka::offset{99},
    };
    ...
    ASSERT_TRUE(decoded.imported.has_value());
    ASSERT_EQ(decoded.imported, v1.imported);
}
...
TEST(ImportedSerdeTest, ObjectEntryV1CompatV2) {
    ...
    ASSERT_EQ(v2.imported, std::nullopt);
}
...
TEST(ImportedSerdeTest, ObjectEntryV2RoundTrip) {
    ...
    v2.imported = imported_segment_info{
      .ts_path = "00000000000000000000-1-v1.log",
      .delta_offset = model::offset_delta{3},
      .delta_offset_end = model::offset_delta{3},
      .base_kafka_offset = kafka::offset{200},
      .last_kafka_offset = kafka::offset{299},
    };
...
// Verify that imported_segment_info round-trips through serde correctly.
TEST(ImportedSerdeTest, ImportedSegmentInfoRoundTrip) {
    imported_segment_info info{
      .ts_path = "00000000000000000000-1-v1.log",
      .delta_offset = model::offset_delta{7},
      .delta_offset_end = model::offset_delta{8},
      .base_kafka_offset = kafka::offset{500},
      .last_kafka_offset = kafka::offset{599},
    };

    iobuf buf = serde::to_iobuf(info);
    auto decoded = serde::from_iobuf<imported_segment_info>(std::move(buf));
    ASSERT_EQ(decoded, info);
}

namespace {

/// Mirrors metadata_row_value at version<1>, before the `migration_phase`
...
/// Mirrors partition_state at version<0>, before the `migration_phase` field
...
// A v1 metadata_row_value (no `migration_phase`) decodes as the current v2
// struct with migration_phase defaulting to `none`.
TEST(ImportedSerdeTest, MetadataRowValueV1CompatV2) {
    ...
    ASSERT_EQ(v2.migration_phase, migration_phase::none);
}

// A v2 metadata_row_value with migration_phase set round-trips through serde.
TEST(ImportedSerdeTest, MetadataRowValueV2RoundTrip) {
    ...
    v2.migration_phase = migration_phase::migrating;
    ...
    ASSERT_EQ(decoded.migration_phase, migration_phase::migrating);
}

// A v0 partition_state (no `migration_phase`) decodes as the current v1 struct
// with migration_phase defaulting to `none`.
TEST(ImportedSerdeTest, PartitionStateV0CompatV1) {
    ...
    ASSERT_EQ(v1.migration_phase, migration_phase::none);
}

// A v1 partition_state with migration_phase set round-trips through serde.
TEST(ImportedSerdeTest, PartitionStateV1RoundTrip) {
    partition_state ps;
    ps.start_offset = kafka::offset{10};
    ps.next_offset = kafka::offset{42};
    ps.migration_phase = migration_phase::complete;
    ...
    ASSERT_EQ(decoded.migration_phase, migration_phase::complete);
}
