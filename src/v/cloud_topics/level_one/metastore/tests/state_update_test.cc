...
protected:
    ...
    // append_imported_objects interface adapter: a batch that does not connect
    // at the tail (non-empty corrections) is reported as an error rather than
    // silently dropped.
      ...
      kafka::offset last) {
        ...
        for (auto& [tid, parts] : o.extent_metas) {
            for (auto& [p, meta] : parts) {
                meta.imported_ts_delta = imported_ts_segment_info{};
            }
        }
        return o;
    }
