...
fake_io::open_object(
  object_extent extent, ss::abort_source* as, cloud_io::group_id g) {
    if (extent.imported.has_value()) {
        ...
        // 0 with the base delta -- file_io's missing-.index behavior.
        ...
        auto idx = std::make_unique<ts_segment_index>(
          std::move(oi),
          fixture.last_kafka_offset,
          fixture.delta_offset,
          segment_size);
  ...
  chunked_vector<object_location> objects, ss::abort_source*) {
    for (const auto& obj : objects) {
        if (obj.ts_path.has_value()) {
            // Imported object: its backing segment lives in the TS bucket,
            // addressed by ts_path (mirrors file_io's bucket routing).
...
bool fake_io::has_ts_segment(const ss::sstring& ts_path) const {
