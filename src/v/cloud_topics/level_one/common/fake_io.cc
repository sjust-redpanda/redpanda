  ...
  bool skip_cache) {
    if (extent.imported.has_value()) {
        ...
        aborted_transactions aborted = extent.imported->tx_state
                                         ...
                                         : fixture.aborted;
        // Use a deliberately small chunk size so tests that read imported
        // extents through fake_io exercise the chunked data source for real
        // -- multi-chunk reads and batches spanning chunk boundaries -- rather
        // than the whole-suffix fallback. The in-memory fetch serves any
        // [pos,len); bytes is held via a shared pointer so the fetch is
        // copyable (ts_fetch_range_fn), matching file_io.
        static constexpr size_t fake_chunk_size = 64;
        co_return std::make_unique<ts_object_handle>(
          std::move(idx),
          extent.imported->segment_term,
          std::move(aborted),
          [bytes = ss::make_lw_shared<iobuf>(fixture.bytes.copy())](
            size_t pos, size_t len, ss::abort_source*)
            -> ss::future<std::expected<ss::input_stream<char>, io::errc>> {
    ...
    auto stream_result = co_await read_object(extent, as, g, skip_cache);
