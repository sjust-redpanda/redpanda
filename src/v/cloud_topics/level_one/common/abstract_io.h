...
public:
    ...
    // L1 footer and implements the native seek path.
    // For imported TS extents (extent.imported.has_value()), the handle loads
    // the TS segment index and translates offsets via delta_offset.
    virtual ss::future<std::expected<std::unique_ptr<object_handle>, errc>>
    open_object(object_extent, ss::abort_source*) = 0;

    // Delete the specified objects from object storage.
    //
    // Each extent carries optional imported_segment_info. When present the
    // routing layer directs the deletion to the TS bucket; otherwise the L1
    // bucket path is used.
    virtual ss::future<std::expected<void, errc>>
    delete_objects(chunked_vector<object_extent>, ss::abort_source*) = 0;
