...
std::optional<seek_result> ts_segment_index::seek_to_offset(
  model::topic_id_partition, kafka::offset target) const {
    ...
    model::offset_delta delta
      = entry ? model::offset_delta{entry->rp_offset() - entry->kaf_offset()}
              : _base_delta;
    return seek_result{
...
ts_object_handle::open_reader(
  const seek_result& seek, ss::abort_source* as) {
    auto delta = seek.delta.value_or(_base_delta);
    ...
    co_return std::make_unique<tiered_storage_object_reader>(
      std::move(*stream_result), delta, _term, _aborted);
}
