...
// to {file_position=0, length=segment_size, base delta} -- a full-segment scan.
TEST(OpenObjectTsTest, SeekReturnsFullSegment) {
    ...
    object_extent extent{
      ...
      .size = segment_size,
      .imported
      = imported_ts_info{.ts_path = ts_path, .delta_offset = delta, .delta_offset_end = delta, .last_kafka_offset = 9_o},
    };
    auto handle_result
      = fio.open_object(extent, &as, cloud_io::group_id::default_group).get();
    ASSERT_TRUE(handle_result.has_value());
...
TEST(OpenObjectTsTest, ReadBatchesWithDeltaTranslation) {
    ...
    object_extent extent{
      ...
      .size = sz,
      .imported
      = imported_ts_info{.ts_path = ts_path, .delta_offset = delta, .delta_offset_end = delta, .last_kafka_offset = 14_o},
    };
    auto handle_result
      = fio.open_object(extent, &as, cloud_io::group_id::default_group).get();
    ASSERT_TRUE(handle_result.has_value());
...
// the delta from the indexed entry, and the reader resumes at that position.
// Three raft_data batches, delta 5 (log = kafka + 5):
...
TEST(OpenObjectTsTest, IndexBackedSeekFindsBatchPosition) {
    ...
    fio.put_ts_segment(
      ts_path, std::move(segment), 0_o, 14_o, delta, {}, oi.to_iobuf());
    ...
    // with the delta recomposed from the indexed entry (rp 10 - kaf 5 = 5).
    auto seek = handle->index().seek_to_offset(tidp, 5_o);
    ASSERT_TRUE(seek.has_value());
...
TEST(OpenObjectTsTest, ReadImportedExtentEmitsOnlyRaftData) {
    ...
    object_extent extent{
      ...
      .size = sz,
      .imported
      = imported_ts_info{.ts_path = ts_path, .delta_offset = model::offset_delta{0}, .delta_offset_end = model::offset_delta{0}, .last_kafka_offset = 3_o},
    };
    auto handle_result
      = fio.open_object(extent, &as, cloud_io::group_id::default_group).get();
    ASSERT_TRUE(handle_result.has_value());
...
TEST(OpenObjectTsTest, ReadImportedExtentDropsControlBatches) {
    ...
    object_extent extent{
      ...
      .size = sz,
      .imported
      = imported_ts_info{.ts_path = ts_path, .delta_offset = model::offset_delta{0}, .delta_offset_end = model::offset_delta{0}, .last_kafka_offset = 5_o},
    };
    auto handle_result
      = fio.open_object(extent, &as, cloud_io::group_id::default_group).get();
    ASSERT_TRUE(handle_result.has_value());
...
TEST(OpenObjectTsTest, ReadImportedExtentAllAborted) {
    ...
    object_extent extent{
      ...
      .size = sz,
      .imported
      = imported_ts_info{.ts_path = ts_path, .delta_offset = model::offset_delta{0}, .delta_offset_end = model::offset_delta{0}, .last_kafka_offset = 4_o},
    };
    auto handle_result
      = fio.open_object(extent, &as, cloud_io::group_id::default_group).get();
    ASSERT_TRUE(handle_result.has_value());
