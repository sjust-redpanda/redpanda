...
TEST(OpenObjectTest, SeekReturnsNonzeroPosition) {
    ...
    auto handle_result
      = fio.open_object(extent, &as, cloud_io::group_id::default_group, false)
          .get();
...
TEST(OpenObjectTsTest, ReadImportedExtentTxAbsentSkipsAbortStripping) {
    fake_io fio;
...
TEST(OpenObjectTsTest, MissingTsSegmentReturnsError) {
    ...
    object_extent extent{
      ...
      .size = 100,
      .imported
      = imported_ts_info{.ts_path = ts_segment_path{"no-such-segment.log"}},
    };
    auto result
      = fio.open_object(extent, &as, cloud_io::group_id::default_group, false)
          .get();
