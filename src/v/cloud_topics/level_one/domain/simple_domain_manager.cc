...
simple_domain_manager::get_offsets(rpc::get_offsets_request req) {
    ...
    co_return rpc::get_offsets_reply{
      ...
      .migration_phase = get_res->migration_phase,
    };
}
...
simple_domain_manager::set_migration_phase(
  rpc::set_migration_phase_request req) {
    ...
    auto sync_res = co_await stm_->sync(10s);
    if (!sync_res.has_value()) {
        co_return rpc::set_migration_phase_reply{
          .ec = convert_stm_errc(sync_res.error())};
    }
    bool is_no_op = false;
    auto update_res = set_migration_phase_update::build(
      stm_->state(), req.tp, req.phase, &is_no_op);
    if (!update_res.has_value()) {
        vlog(
          cd_log.debug,
    ...
    if (is_no_op) {
        co_return rpc::set_migration_phase_reply{.ec = rpc::errc::ok};
    }
    storage::record_batch_builder builder(
      model::record_batch_type::l1_stm, model::offset{0});
    builder.add_raw_kv(
      serde::to_iobuf(set_migration_phase_update::key),
      serde::to_iobuf(std::move(update_res.value())));
    auto repl_res = co_await stm_->replicate_and_wait(
      sync_res.value(), std::move(builder).build(), as_);
    if (!repl_res.has_value()) {
