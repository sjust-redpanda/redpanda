        ...
        std::optional<partition_state> state) noexcept {
          ...
          case notif_type::partition_properties_change:
              if (t == notif_type::partition_properties_change && is_leader) {
                  // A properties change on an already-leading broker does
                  // not trigger on_leadership_change (reconciler/housekeeper
                  // attach callbacks) via the fallthrough path above. Fire
                  // it explicitly so they attach immediately after a
                  // storage-mode transition. Callbacks are idempotent.
                  on_leadership_change(ntp, tidp, is_leader);
              }
