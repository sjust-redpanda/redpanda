          ...
          [this, &r](
            ...
            auto partition) noexcept {
              if (partition) {
                  ...
                  // (partition_storage_mode is still tiered) the reconciler
                  // skips it each round (!source::is_cloud_topic()), so the
                  // archiver's mirror is the sole L1 writer; once cutover
                  // advances partition_storage_mode to cloud the reconciler
                  // picks it up on the next round without a re-attach.
                  // Leadership notifications do not fire on cutover, so a
                  // per-round skip -- not an attach-time gate -- is what makes
                  // the transition take effect.
                  r.attach_partition(
