...
public:
    ...
    // L1 mirror has dropped its reference to a tiered-storage object before that
    // object is deleted (the mirror must never reference a removed object, the
    // same invariant the archival manifest upholds in garbage_collect). Detaches
    // the imported extents below the true log start; if retention has run ahead
    // of what the mirror imported, empties the mirrored range so the forward
    // append re-seeds it at the surviving manifest start. No-op unless migrating.
