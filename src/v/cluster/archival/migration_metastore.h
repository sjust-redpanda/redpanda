...
public:
    ...
    /// One tiered-storage segment to register as an imported L1 extent. Mirrors
    /// the fields of cloud_topics' imported_segment_info plus the per-segment
    /// term/timestamp/size needed to build the metastore extent. The owning
    /// partition is passed alongside (the archiver addresses by ntp; the sink
    /// resolves ntp -> the L1 topic_id_partition).
