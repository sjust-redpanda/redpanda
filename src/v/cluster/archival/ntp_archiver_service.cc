...
bool ntp_archiver::is_cloud_topic_dormant() const {
    ...
    // has cut over -- partition_storage_mode advanced to cloud and the archival
    // STM emptied -- the archiver must go dormant. Otherwise it would re-upload
    // the (not-yet-trimmed) raft log, re-populating the manifest, which
    // re-triggers the migration: the partition would oscillate instead of
    // settling as a native cloud topic, and post-cutover writes would never
    // reach the cloud-topic (L0) path. cloud_topic_enabled() keys on
    // partition_storage_mode, which stays tiered throughout migration, so it
    // becomes true only at cutover; has_archived_data() then covers the brief
    // window where partition_storage_mode has advanced but reset_metadata has
    // not yet committed.
    //
...
ntp_archiver::schedule_uploads(model::offset max_offset_exclusive) {
    ...
    // partition_storage_mode is still tiered during migration, so
    // cloud_topic_enabled() below stays false and would not suspend us on its
    // own.
    if (
...
ss::future<> ntp_archiver::run_migration_mirror() {
    ...
    const auto& m = manifest();
    const bool has_archived = stm->has_archived_data();
    ...
    // upload racing the cutover; and once partition_storage_mode advances to
    // cloud and the manifest is emptied, may_begin_uploads is false
