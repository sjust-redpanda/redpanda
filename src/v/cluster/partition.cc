...
ss::future<> partition::cutover_to_cloud_topic(
  ...
  std::optional<model::offset> log_boundary) {
    ...
    vlog(
      ...
      "partition_storage_mode to cloud, emptying archival STM",
      _raft->ntp(),
      boundary,
      log_boundary);
    ...
    // partition_storage_mode. Advancing partition_storage_mode is the routing
    // flip (the read path keys on cloud_topic_enabled() ->
    // partition_storage_mode); it must never expose "neither STM
    // authoritative", so ctp_stm has to be seeded while the partition is still
    // routed to tiered storage. At convergence the L1 mirror provably covers
    // the manifest tail, so [0, B] is reconciled.
    ...
    // Step 2: advance partition_storage_mode tiered->cloud. THIS is the routing
    // flip: the structural read/write path keys on cloud_topic_enabled(), which
    // reads partition_storage_mode. ctp_stm is now authoritative, so the flip
    // is safe. It must precede reset_metadata -- with partition_storage_mode
    // still tiered a reader would route to the about-to-be-emptied manifest and
    // miss the migrated prefix.
    auto pm = co_await _partition_properties_stm->set_partition_storage_mode(
      model::redpanda_storage_mode::cloud);
    if (pm.has_error()) {
        vlog(
          clusterlog.warn,
          "{}: TS->CT cutover: failed to advance partition_storage_mode to "
          "cloud; will "
          "retry: {}",
          _raft->ntp(),
          pm.error().message());
...
partition::finish_cutover_tail(ss::lowres_clock::time_point deadline) {
    ...
    // partition_storage_mode, which was still tiered, so it reaches the
    // eviction and archival STMs but not the ctp_stm -- and reset_metadata
    // below erases the archival manifest's copy while the cloud-topic read path
    // consults only the ctp_stm's start offset. Without this the truncation is
    // acknowledged and then silently undone: the cut-over partition serves from
    // the imported extents' base offset again. set_start_offset is monotonic,
    // so repeating the handover is free when nothing landed. Runs before
    // reset_metadata so both carriers of the override are still intact.
    if (!co_await hand_over_trim_floor(deadline)) {
        co_return;
    }

    // Empty the archival manifest: makes the (now cloud-mode) archiver
    // dormant, which keys on has_archived_data().
    if (_archival_meta_stm && _archival_meta_stm->has_archived_data()) {
...
ss::future<> partition::maybe_finish_cutover() {
    ...
    // partition_storage_mode to cloud (the routing flip) before emptying the
    // archival manifest; a crash in between leaves
    // partition_storage_mode==cloud (routing already correct, on the
    // cloud-topic path) with a non-empty manifest that no archiver will clean
    // up (should_construct_archiver() is false for a cloud-mode partition). On
    // becoming leader, finish the tail. ctp_stm was seeded before
    // partition_storage_mode advanced (the cutover invariant), so it is already
    // authoritative; only the archival-STM reset + mark_complete remain.
    ...
    if (
      _partition_properties_stm->partition_storage_mode()
        != model::redpanda_storage_mode::cloud
      || !_archival_meta_stm->has_archived_data()) {
        co_return;
    }
    vlog(
      clusterlog.info,
      "{}: finishing interrupted TS->CT cutover (partition_storage_mode cloud, "
      "manifest "
      "still non-empty)",
      _raft->ntp());
    auto deadline = ss::lowres_clock::now()
