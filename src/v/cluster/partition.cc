...
bool partition::should_construct_archiver() {
    ...
    const bool base = config::shard_local_cfg().cloud_storage_enabled()
                      ...
                      // topic is not exempt from this. It should be possible
                      // to create an archiver for it.
                      && _raft->ntp().ns == model::kafka_namespace
                      && _raft->ntp().tp.topic
    ...
    // Cloud topic: keep the archiver alive while there are pre-migration TS
    // segments that still need GC via apply_retention() + garbage_collect().
    // Once the archival manifest is drained, should_construct_archiver returns
    // false and the archiver is not rebuilt on the next restart_archiver call.
    ...
    const kafka::offset boundary
      = _archival_meta_stm
          ? _archival_meta_stm->manifest().get_last_kafka_offset().value_or(
              kafka::offset::min())
          : kafka::offset::min();
    if (boundary == kafka::offset::min()) {
        // No TS data has been uploaded, so there is nothing to serve via the
        // TS passthrough read path.  Skip setting a migration boundary; the
        // partition operates in pure CT mode from the start.
        co_return;
    }
