...
ss::future<consensus_ptr> partition_manager::manage(
  ...
  std::optional<partition_bootstrap_params> bootstrap_params) {
    ...
    // partition_storage_mode must load as `tiered` so the partition is served
    // as TS until the resumed mirror cuts over. The leadership bootstrap cannot
    // classify this reliably -- the archival manifest rebuilds asynchronously,
    // so has_archived_data() may still read false and misrecord `cloud`, and
    ...
    if (
      ...
      && log->config().recovery_enabled()
        ...
        co_await partition_properties_stm::seed_partition_storage_mode(
          _storage.kvs(),
          clusterlog,
          log->config().ntp(),
