...
public:
    ...
    // restored with a non-empty log comes up with partition_storage_mode
    // already set -- before any leadership/config-propagation race could
    // misclassify it. The seed survives persisted_stm start, whose initial
    // cleanup only runs for an empty log (dirty_offset == {}), whereas a
    // restored/bootstrapped partition has a non-empty log. `snapshot_offset`
    // is the last-applied offset the seed represents (prev(log start)).
    static ss::future<> seed_partition_storage_mode(
      storage::kvstore& kvstore,
      ss::logger& logger,
      const model::ntp& ntp,
