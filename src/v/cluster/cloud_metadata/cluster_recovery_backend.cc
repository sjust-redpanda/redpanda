            ...
            if (topic_cfg.is_read_replica()) {
                ...
                if (metastore && topic_cfg.tp_id.has_value()) {
                    ...
                    for (int32_t p = 0; p < topic_cfg.partition_count; ++p) {
                        ...
                        // manifest, is served as TS because
                        // partition_storage_mode is tiered (routing keys on
                        // partition_storage_mode, not the manifest), and the
                        // live migration mirror resumes and cuts over.
                        //
                ...
                if (topic_is_migrating) {
                    ...
                    // would wake the dormant archiver (has_archived_data()
                    ...
                    // keys on partition_storage_mode, which stays cloud.)
