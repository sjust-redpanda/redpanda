            ...
            if (topic_cfg.is_read_replica()) {
                ...
                if (metastore && topic_cfg.tp_id.has_value()) {
                    ...
                    for (int32_t p = 0; p < topic_cfg.partition_count; ++p) {
                        ...
                        if (!offsets_res.has_value()) {
                            if (
                              ...
                              == cloud_topics::l1::metastore::errc::
                                missing_ntp) {
                                // The partition is absent from the L1 metastore.
                                // For a cloud topic that still carries
                                // tiered-storage data (shadow_indexing/archival
                                // from its pre-migration tiered config), this
                                // means it has not cut over -- a completed
                                // migration would be present in L1. Its
                                // authoritative data is in tiered storage, so
                                // recover it as tiered storage (rebuild the
                                // archival STM from the remote manifest);
                                // ignore the absent L1 state, the migration
                                // mirror rebuilds L1 after recovery. A native
                                // cloud topic (no tiered-storage config) is a
                                // genuinely empty new partition.
                                const auto& si
                        ...
                        // manifest, is served as TS via the structural gate,
                        // and the live migration mirror resumes and cuts over.
                        //
                        // Do NOT give it cluster-recovery bootstrap params:
                        // those route partition_manager::manage() to the
                        // cloud-topic ctp_stm bootstrap, which seeds an empty L1
                        // and skips the remote-manifest download. Leaving the
                        // bootstrap params unset routes the partition to the
                        // tiered-storage recovery path (maybe_download_log +
                        // archival_metadata_stm::make_snapshot), which restores
                        // the archival manifest. The ctp_stm starts empty (as it
                        // does for a freshly created cloud topic) and the resumed
                        // migration mirror rebuilds L1 from the restored
                        // manifest.
                ...
                if (topic_is_migrating) {
                    ...
                    // would make the structural IO gate route the partition
                    // back to tiered storage, and the (non-dormant) archiver
                    // would treat it as live -- re-triggering migration (case
                    // C) and applying retention to the imported segments.
                    // Imported extents carry their own tiered-storage paths
                    // (imported_ts_info::ts_path), so they stay readable from L1
