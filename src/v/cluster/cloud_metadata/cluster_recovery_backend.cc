            ...
            if (topic_cfg.is_read_replica()) {
                ...
                if (metastore && topic_cfg.tp_id.has_value()) {
                    ...
                    for (int32_t p = 0; p < topic_cfg.partition_count; ++p) {
                        ...
                        if (
                          offsets_res->migration_phase
                          == cloud_topics::l1::migration_phase::migrating) {
                ...
                if (topic_is_migrating) {
                    ...
                    // (imported_segment_info::ts_path), so they stay readable
                    // from L1 without these properties.
