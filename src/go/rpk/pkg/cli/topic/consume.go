...
type consumer struct {
	...
	useTopicSchemaContext bool   // --use-topic-schema-context; derive the context from topic config
...
func newConsumeCommand(fs afero.Fs, p *config.Params) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, topics []string) {
			...
			if len(c.useSchemaRegistry) > 0 {
				...
				//     --use-topic-schema-context is set (reads topic config via DescribeConfigs);
				//     else
				//   - the --schema-context flag value (empty or "." = default context).
				// The two flags are mutually exclusive. A single consume can span topics in
				// different contexts, so this resolves per topic (cached). Shares helpers with
				// `rpk topic produce`.
				...
				c.resolveCtx = func(topic string) string {
					...
					var ctx string
					if c.useTopicSchemaContext && topic != "" {
						...
						ctx = normalizeSchemaContext(topicCtx)
					} else {
						ctx = normalizeSchemaContext(c.schemaContext)
					}
					ctxCache[topic] = ctx
					return ctx
				}
	...
	cmd.Flags().StringVar(&c.schemaContext, "schema-context", "", "Schema Registry context to resolve schemas in when decoding; defaults to the default context (a lone dot '.' or an empty value also selects it). Mutually exclusive with --use-topic-schema-context")
	cmd.Flags().BoolVar(&c.useTopicSchemaContext, "use-topic-schema-context", false, "Decode schemas in the context bound to each topic via its redpanda.schema.registry.context config; reads topic config with DescribeConfigs (requires the DESCRIBE_CONFIGS ACL). Mutually exclusive with --schema-context")
