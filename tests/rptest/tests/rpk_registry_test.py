        ...
        json_with_reference = """
{
   ...
   "properties":{
    ...
    def test_produce_schema_context_from_topic_config(self):
        # With --use-topic-schema-context, produce derives the context from the topic's
        ...
        # --use-topic-schema-context: rpk reads the topic config -> .team -> stamps the .team id
        # (not default_id, confirming the context came from the topic config rather than the
        # default). Explicit --schema-context selection is covered by
        # test_produce_schema_context_explicit_flag.
        self._rpk.produce(
            topic,
            key="k",
            msg='{"f1":"hi"}',
            schema_id=ctx_id,
            use_topic_schema_context=True,
        )
        assert self._wire_schema_id(topic, offset="0:1") == ctx_id
    ...
    def test_consume_schema_context_from_topic_config(self):
        # With --use-topic-schema-context, consume derives the context from the topic config.
        avro = '{"type":"record","name":"r","fields":[{"name":"f1","type":"string"}]}'
        filler = (
            '{"type":"record","name":"filler","fields":[{"name":"x","type":"string"}]}'
        )
        ...
        # Produce the .team record (config-derived context) and decode it back the same way.
        # default_id exists only to assert ctx_id is distinct, proving the decode context came
        # from the topic config. Explicit --schema-context decoding is covered by
        # test_consume_schema_context_explicit_flag.
        msg = '{"f1":"hi"}'
        self._rpk.produce(
            topic, key="k", msg=msg, schema_id=ctx_id, use_topic_schema_context=True
        )  # config-derived
        out = self._rpk.consume(
            topic,
            offset="0:1",
    ...
    def test_consume_multi_context_no_cache_collision(self):
        ...
        for v in ("y1", "y2"):
            self._rpk.produce(
                "ctxcache-b",
                key="k",
                msg=json.dumps({"bfield": v}),
                schema_id=id_b,
                use_topic_schema_context=True,
            )

        # One consume across both topics (regex) -> a shared decode cache. Each topic's context
        # is derived from its config via --use-topic-schema-context.
        out = self._rpk.consume(
            "ctxcache-.*",
            regex=True,
            n=4,
    ...
    @cluster(num_nodes=3)
    def test_schema_context_flags_mutually_exclusive(self):
        # --schema-context and --use-topic-schema-context select the context two different ways;
        # setting both is rejected at flag-parse time (cobra marks them mutually exclusive), for
        # both produce and consume. rpk exits non-zero and the error names both flags.
        topic = "ctx_mutex"
        self._rpk.create_topic(topic)
        ...
        def rejects_both(e):
            ...
            return (
                bool(e.returncode)
                and "schema-context" in s
                and "use-topic-schema-context" in s
            )

        with expect_exception(RpkException, rejects_both):
            self._rpk.produce(
                topic,
                key="k",
                msg='{"f1":"hi"}',
