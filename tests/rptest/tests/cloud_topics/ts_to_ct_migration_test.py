...
written for):
  ...
  * When the mirror has covered the whole manifest the partition *cuts over* in a
    single step that empties the archival STM (routing then flips to cloud
    topics, serving the imported L1 extents) and seeds the CT reconciliation
    baseline. Cutover is observable here as the archival manifest going empty.
...
class TsToCtMigrationTest(RedpandaTest):
    ...
    def _enable_migration(self):
        """Activate the cluster feature that permits the tiered->cloud
        migration trigger (the storage_mode transition is gated on it)."""
    ...
    def _trigger_migration(self, topic: str, storage_mode: str):
        """Flip the storage mode -- the migration trigger."""
        self.rpk.alter_topic_config(
            topic, TopicSpec.PROPERTY_STORAGE_MODE, storage_mode)

    def _wait_for_cutover(self, topic: str):
        """The cutover empties the archival STM manifest (reset_metadata),
        which flips routing to the cloud-topic path. Observe it as the archival
        partition manifest becoming empty."""
        def cut_over() -> bool:
            manifest = self.admin.get_partition_manifest(topic, 0)
    ...
    def test_ts_to_ct_migration_trigger_feature_gated(self):
        ...
        tiered_to_cloud_migration cluster feature: rejected while the feature is
    ...
    def test_ts_to_ct_migration_no_archived_segments_local_data(self):
        ...
        The structural routing gate keys on the archival manifest: empty +
        cloud_topic_enabled routes straight to the cloud-topic path, never
        through replicated_partition, and the mirror skips an empty manifest so
        there is no import and no cutover. The produced-but-unuploaded records
        live only in the local raft log at the trigger. They must not be
        stranded by the routing flip: a consumer from offset 0 must still see
        every record in order, and they must reconcile into L1 (the durable
        cloud-topic path) rather than only surviving in the local log."""
        self._enable_migration()
        self.rpk.create_topic(
    ...
    def test_ts_to_ct_migration_no_archived_segments_empty_topic(self):
        ...
        any data -- empty log, empty archival manifest.

        The mirror has nothing to import and there is no cutover; the partition
        routes to the cloud-topic path immediately. Data produced after the
    ...
    def test_ts_to_ct_migration_head_prune(self, storage_mode: str):
        ...
        try:
            ...
            # Retention must keep trimming the local log THROUGHOUT migration:
            # the archival manifest head advances (cloud retention) and the
            # partition's kafka start offset must track it (local retention),
            # which is what drives the mirror's head-prune of the L1 imported
            # extents. This is the invariant, not "kafka start moved once" -- a
            # single pre-migration trim used to satisfy that while local
            # retention was in fact dead for the rest of the migration.
            #
            # Once migration is well underway (manifest head past
            # STALL_MANIFEST_START), the local start must stay within MAX_TRIM_LAG
            # of it. A dead/stalled local retention leaves the kafka start frozen
            # while the manifest races ahead, so the lag diverges unboundedly and
            # we fail immediately with a clear assertion rather than an opaque
            # timeout. Healthy lag is a few thousand offsets (the local-vs-cloud
            # retention-window difference + catch-up); the dead case is the full
            # manifest start (~10^5).
            STALL_MANIFEST_START = 100_000
            MAX_TRIM_LAG = 50_000
            ...
            def head_trimmed_while_migrating() -> bool:
                try:
                    ...
                    self.logger.debug(f"head_prune poll transient error: {e}")
                    return False
                segs = len(m.get("segments", {}))
                ...
                if segs == 0:
                    ...
                    return False
                if mstart <= STALL_MANIFEST_START:
                    # Wait until migration is well underway before judging that
                    # local trim is tracking the manifest.
                    return False
            ...
            wait_until(
                ...
                timeout_sec=180,
                backoff_sec=3,
                err_msg="manifest head did not advance past "
                f"{STALL_MANIFEST_START} while migrating (could not assess "
                "local-trim tracking)",
            )
        finally:
            producer.stop()
    ...
    def test_ts_to_ct_migration_delete_records(self):
        """DeleteRecords (Kafka prefix truncation) WHILE migrating. Trimming the
        prefix advances the partition start and must head-prune the mirrored L1
        imported extents below the new start (detach only -- the archiver still
        owns the backing tiered-storage objects during migration). After cutover
        the start offset is the trimmed offset (> 0) and a consumer from the
        earliest available offset sees a correct, gap-free prefix: no stale
        extents below the start, no gap above it.

        Distinct from the retention-driven head_prune case -- this is the
        explicit DeleteRecords API path into the same trim-floor handoff. Cutover
...
class TsToCtMigrationRecoveryTest(RedpandaTest):
    ...
    def test_ts_to_ct_migration_recovery(self):
        ...
        # phase is migrating and cutover is held off), served via the structural
        # gate from the rebuilt archival STM.
    ...
    def test_recovery_of_completed_migration(self):
        ...
        Regression test for a recovery routing defect. Cutover empties the *live*
        archival manifest, flipping the structural IO gate to the cloud-topic
        path. But the *remote* archival manifest still holds the pre-cutover
