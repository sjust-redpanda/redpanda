...
class TsToCtMigrationReadReplicaTest(RedpandaTest):
    ...
    def test_ts_to_ct_migration_read_replica_live_through_migration(self):
        ...
        try:
            ...
            assert status.offset_gaps == 0, (
                f"replica saw a gap across migration: offset_gaps={status.offset_gaps}"
            )
        finally:
...
class TsToCtMigrationEmptyManifestTest(RedpandaTest):
    ...
    def test_local_data(self):
        ...
        FAILS on current code: run_migration_mirror co_returns on m.size()==0
        before the cutover trigger, so the partition stays migrating and never
        reconciles -> _wait_for_migration_complete times out."""
        self._enable_migration()
        self.rpk.create_topic(
            self.TOPIC_LOCAL,
            partitions=1,
    ...
    def test_empty_topic(self):
        ...
        FAILS on current code for the same reason (mirror skips the empty
        manifest, so no cutover)."""
        self._enable_migration()
        self.rpk.create_topic(
            self.TOPIC_EMPTY,
            partitions=1,
