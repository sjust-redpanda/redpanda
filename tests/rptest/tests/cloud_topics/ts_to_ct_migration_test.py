...
written for):
  ...
  * When the mirror has covered the whole manifest the partition *cuts over*:
    it advances partition_storage_mode to cloud -- which flips routing to cloud topics
    (routing keys on partition_storage_mode), serving the imported L1 extents -- empties
...
class TsToCtMigrationTest(RedpandaTest):
    ...
    def _enable_migration(self):
        ...
        transition is gated on both the topic_storage_mode_migration cluster feature and
        the enable_topic_storage_mode_migration config; set both."""
        self.redpanda.set_feature_active("topic_storage_mode_migration", True, timeout_sec=30)
        self.redpanda.set_cluster_config({"enable_topic_storage_mode_migration": True})
    ...
    def _wait_for_cutover(self, topic: str):
        """Cutover advances partition_storage_mode to cloud -- which flips routing to
        the cloud-topic path, since routing keys on partition_storage_mode (the
    ...
    def test_ts_to_ct_migration_trigger_feature_gated(self):
        ...
        topic_storage_mode_migration cluster feature: rejected while the feature is
        ...
        try:
            ...
            raise AssertionError(
                ...
                "topic_storage_mode_migration feature was inactive"
            )
        except RpkException:
            pass
    ...
    def test_ts_to_ct_migration_no_archived_segments_local_data(self):
        ...
        Routing keys on partition_storage_mode (the partition_properties field), not on
        the archival manifest: while migrating, partition_storage_mode lags at tiered so
        reads are served via replicated_partition; cutover advances
        partition_storage_mode to cloud, flipping routing to the cloud-topic path. The
    ...
    def test_ts_to_ct_migration_no_archived_segments_empty_topic(self):
        ...
        Routing keys on partition_storage_mode (the partition_properties field): during
        migration partition_storage_mode lags at tiered, and cutover advances it to
...
class TsToCtMigrationRecoveryTest(RedpandaTest):
    ...
    def __init__(self, test_context: TestContext):
        ...
        super().__init__(
            ...
            si_settings=si_settings,
            extra_rp_conf={
                "enable_topic_storage_mode_migration": True,
                "cloud_topics_produce_batching_size_threshold": 65536,
                "log_segment_size_min": 1,
                "log_segment_ms_min": 1000,
                "log_segment_size": 1048576,
    ...
    def test_ts_to_ct_migration_recovery(self):
        ...
        # phase is migrating and cutover is held off): partition_storage_mode is tiered,
    ...
    def test_ts_to_ct_migration_recovery_trimmed_head(self):
        ...
        def describe_start() -> int | None:
            # None when describe reports no partition at all, so that case stays
            # distinguishable from a partition genuinely starting at 0.
            parts = list(self.rpk.describe_topic(self.TOPIC_TRIMMED))
            return parts[0].start_offset if parts else None
        ...
        try:
            ...
            def trimmed_migrating() -> bool:
                ...
                migrating = len(m.get("segments", {})) > 0
                start = describe_start()
                return migrating and start is not None and start >= trim_to
        ...
        # Recovery reporting `inactive` only means the controller state is back;
        # the recovered partition surfaces its start offset afterwards, on its own
        # schedule. So this converges rather than holding the instant recovery
        # finishes -- read once and it reports the pre-restore 0.
        seen: list[int | None] = []

        def trimmed_start_restored() -> bool:
            start = describe_start()
            seen.append(start)
            return start is not None and start >= trim_to

        wait_until(
            trimmed_start_restored,
            timeout_sec=60,
            backoff_sec=1,
            err_msg=lambda: (
                f"trimmed start lost on recovery: start={seen[-1] if seen else None}"
                f" < {trim_to} (start=None means describe reported no partition)"
            ),
            retry_on_exc=True,
        )
        recovered_start = describe_start()
        assert recovered_start is not None, "describe reported no partition"
    ...
    def test_recovery_of_completed_migration(self):
        ...
        partition_storage_mode to cloud -- flipping routing to the cloud-topic path,
        since routing keys on partition_storage_mode -- and empties the *live* archival
    ...
    def test_read_replica_storage_mode_alter_rejected(self):
        ...
        try:
            ...
            self.second_cluster.set_feature_active(
                "topic_storage_mode_migration", True, timeout_sec=30
            )
            rr_rpk = RpkTool(self.second_cluster)
            rr_rpk.create_topic(
                self.TOPIC,
