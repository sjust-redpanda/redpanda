...
class ManualFinalizationUpgradeTest(UnfinalizedUpgradeMixin, FeaturesTestBase):
    ...
    def _perturb_v26_2_to_v26_3(self, phase):
        ...
        if "upgraded" not in phase:
            return
        self._exercise_topic_storage_mode_migration()
    ...
    def _exercise_topic_storage_mode_migration(self):
        """topic_storage_mode_migration gate: a tiered -> cloud storage-mode change
        migrates the topic's data to the new mode, and is permitted only when
        this feature is active AND enable_topic_storage_mode_migration is on. While the
        ...
        downgrade-safe, since a migration would replicate a partition_storage_mode
        command the prior release cannot decode.

        Assert the feature side of that conjunction, and that the config alone
        does not open the gate: turning enable_topic_storage_mode_migration on must
        ...
        assert self._feature_state("topic_storage_mode_migration") == "unavailable", (
            "topic_storage_mode_migration should be unavailable while the upgrade "
            "is unfinalized"
        )
        self.admin.patch_cluster_config(upsert={"enable_topic_storage_mode_migration": True})
        try:
            assert self._feature_state("topic_storage_mode_migration") == "unavailable", (
                "enabling enable_topic_storage_mode_migration must not open the feature gate "
            ...
            self.admin.patch_cluster_config(remove=["enable_topic_storage_mode_migration"])
    ...
    def _verify_topic_storage_mode_migration_working(self):
        ...
        does not configure.)"""
        wait_until(
            lambda: self._feature_state("topic_storage_mode_migration") == "active",
            timeout_sec=30,
            backoff_sec=1,
            err_msg="topic_storage_mode_migration did not auto-activate after finalize",
        )
    ...
    def _verify_v26_3_features_working(self):
        ...
        self._verify_topic_storage_mode_migration_working()
